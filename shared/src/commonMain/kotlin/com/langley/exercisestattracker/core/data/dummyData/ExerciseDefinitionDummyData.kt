...
class ExerciseDefinitionDummyData {
    ...
    fun convertDummyDataToExerciseDef(dummyDataList: List<Map<String, String>>): MutableList<ExerciseDefinition>{

        val convertedList = mutableListOf<ExerciseDefinition>()

        for (exercise in dummyDataList){
    ...
    for (i in 0..100){
        ...
        dummyExerciseRecords.add(
            ExerciseRecord(
                ...
                weightUsed = Random.nextInt(300).toFloat() * randomDef.isWeighted,
                isCardio = randomDef.isCardio.toInt() == 1,
                isCalisthenic = randomDef.isCalisthenic.toInt() == 1,
                duration = randomDef.defaultDuration.toFloat(),
                distance = randomDef.defaultDistance.toFloat(),
                repsCompleted = Random.nextInt(20) * randomDef.hasReps.toInt(),
